...
(defun pertbot-handle-follow (nickuserhost msg)
  (if (string-match "^\\s *,follow\\s +\\(\\S +\\)\\s +\\(\\S +-\\S +*\\)\\s *\n" msg)
      (let* ((followed (erc-downcase (match-string-no-properties 1 msg)))
	     (dir (match-string-no-properties 2 msg))
	     (follower (erc-downcase (car (split-string nickuserhost "!")))))
	(if (member followed (mapcar #'erc-downcase erc-nick))
	    (erc-send-message "Can't follow %s!" followed) ; avoid infinite loop :)
	  (if (member dir (pertbot-installed-dirs))
...
(defun pertbot-handle-follow-match (nickuserhost msg)
  (let* ((followed (erc-downcase (car (split-string nickuserhost "!"))))
